...
bodyweight <- tibble(day = seq(1,360,1)) %>%
  mutate(bw = (bw_0^c_1 + c_2 * day)^c_4,
         bw_g = (bw_0^geometric_c_1 + geometric_c_2 * day)^geometric_c_4) %>%
  pivot_longer(c(bw, bw_g), names_to = "method", values_to = "bw", values_drop_na = TRUE)
...
cray.growth.plot <- bodyweight %>%
  ggplot(aes(day, bw, color = method)) +
  geom_line() +
  lims(x = c(0, 300), y = c(0, 50)) +
  scale_color_discrete(labels = c("arithmetic", "geometric")) +
...
# create a test for loop to make a production plan
